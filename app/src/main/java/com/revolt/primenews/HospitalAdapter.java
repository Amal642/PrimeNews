...
public class HospitalAdapter extends RecyclerView.Adapter<HospitalAdapter.RecyclerViewHolder>{
    ...
    public HospitalAdapter(Context context, List<Taxi> uploads) {
        mContext = context;
        teachers = uploads;
    }
    ...
    @Override
    public HospitalAdapter.RecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(mContext).inflate(R.layout.row_modellink, parent, false);
        return new HospitalAdapter.RecyclerViewHolder(v);
    }
    @Override
    public void onBindViewHolder(final HospitalAdapter.RecyclerViewHolder holder, int position) {
